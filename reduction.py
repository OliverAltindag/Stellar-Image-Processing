...
def stack_all_filters(folder_path, filter_files, all_shifts, pad_val):
    ...
    for filter_name in filter_files.keys():
        ...
        if not file_list or not shifts_list:
            continue
...
def _centroid_and_shift(file_paths, ref_path, star_coords, bg_coords, pad_val):
    '''
    Helper function to abstract the repeated centroiding and shifting logic.
    '''
    ...
    for stack_path in file_paths:  
        files_to_align.append(stack_path)
        if stack_path == ref_path:
            ...
            shifts = mf.centroiding(stack_path, ref_path, star_coords, bg_coords)
...
def reduction(data_folder_path, science_images_folder):
    ...
    # FINALLY WORKED DO NOT CHNAGE THESE PLEASE
    # finding these coordinates was the greatest triumph in this century 
    _centroid_and_shift(file_paths=trimmed_stack_paths,
                        ref_path=trimmed_stack_paths[0],
                        star_coords=[2755, 2880, 1260, 1310],
                        bg_coords=[1920, 1970, 1700, 1750],
                        pad_val=150)

    # Science Images Pass 2 (doing it twice bc they were so far apart before was still slightly off)
    science_pass2_paths = glob.glob(os.path.join(science_folder_path, "aligned_trimmed_master_stack_*.fit"))
    science_pass2_paths.sort()
    _centroid_and_shift(file_paths=science_pass2_paths,
                        ref_path=science_pass2_paths[0],
                        star_coords=[2171, 2201, 2100, 2130],
                        bg_coords=[1400, 1450, 2325, 2375],
                        pad_val=150)

    # Standard Star Images (basically does everything above again witout trimming)
    standard_pass1_paths = glob.glob(os.path.join(standard_folder_path, "master_stack_*.fit"))
    standard_pass1_paths.sort()
    _centroid_and_shift(file_paths=standard_pass1_paths,
                        ref_path=standard_pass1_paths[0],
                        star_coords=[2690, 2765, 1675, 1730],
                        bg_coords=[1475, 1525, 1575, 1625],
                        pad_val=150)

    # Standard Star Images Pass 2 (just another one in the center ish)
    standard_pass2_paths = glob.glob(os.path.join(standard_folder_path, "aligned_master_stack_*.fit"))
    standard_pass2_paths.sort()
    _centroid_and_shift(file_paths=standard_pass2_paths,
                        ref_path=standard_pass2_paths[0],
                        star_coords=[2405, 2455, 1500, 1550],
                        bg_coords=[2240, 2290, 1550, 1605],
                        pad_val=150)
